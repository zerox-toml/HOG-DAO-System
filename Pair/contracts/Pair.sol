...
contract Pair is IPair {
    ...
    function swap(
        ...
        require(to != token0 && to != token1, "IT"); // Pair: INVALID_TO
        ...
        if (amount0Out > 0) {
            ...
            if (token0 == address(0xA5fA5731CfCCC7e7DBD202579D3B1F6ee78403E9)) { // Placeholder HOG token address
                ...
                _safeTransfer(token0, IHOG(token0).taxRecipient(), taxAmount);
            }
        ...
        if (amount1Out > 0) {
            ...
            if (token1 == address(0xA5fA5731CfCCC7e7DBD202579D3B1F6ee78403E9)) { // Placeholder HOG token address
                ...
                _safeTransfer(token1, IHOG(token1).taxRecipient(), taxAmount);
            }
        ...
        uint amount0In = _balance0 > reserve0 - amount0Out
            ? _balance0 - (reserve0 - amount0Out)
            : 0;
        uint amount1In = _balance1 > reserve1 - amount1Out
            ? _balance1 - (reserve1 - amount1Out)
            : 0;
...

        {
            ...
            require(_k(_balance0, _balance1) >= _k(reserve0, reserve1), "K"); // Pair: K
        }
